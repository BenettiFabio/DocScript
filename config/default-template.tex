...
\usepackage{float}      % consente di posizionare immagini con precisione
...
\usepackage{typicons}   % to handle the emojis/icons

$if(draft)$
\usepackage{draftwatermark} % watermark
$endif$

% lascia spazio per il numero nel toc se diventa troppo lungo
...
\fancypagestyle{fancy}
{
    ...
    % If no logo bypass
    $if(LogoFileName)$\fancyhead[L]{\includegraphics[height=40pt]{$LogoFileName$}}$else$~$endif$
...
\title{
$if(LogoFileName)$\includegraphics[width=0.5\textwidth]{$LogoFileName$}$else$~$endif$
\\[1cm]
$if(CompanyProject)$$CompanyProject$$else$~$endif$
\\
$if(CompanyStudyTitle)$$CompanyStudyTitle$$else$~$endif$
\\
}
